...
uint8_t getEU868FrqID(uint32_t frq)
{
    ...
    {          
        if(RegionNvmGroup2->Channels[i].Frequency == frq)
        {
            id = i;
		}
    }
...
bool RegionEU868TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir )
{
    ...
    phyTxPower = RegionCommonComputeTxPower( txPowerLimited, txConfig->MaxEirp, txConfig->AntennaGain ) + 3/*损耗*/;

    // printf("\n\n--[868]---------------- txConfig->TxPower       = %d ----------------\n", txConfig->TxPower);
    // printf("\n\n--[868]---------------- .TxMaxPower             = %d ----------------\n", RegionNvmGroup1->Bands[RegionNvmGroup2->Channels[txConfig->Channel].Band].TxMaxPower);
    // printf("\n\n--[868]---------------- txPowerLimited          = %d ----------------\n", txPowerLimited);
    // printf("\n\n--[868]----------------txConfig->MaxEirp       = %f ---------------\n", txConfig->MaxEirp);
    // printf("\n\n--[868]----------------txConfig->AntennaGain   = %f ---------------\n", txConfig->AntennaGain);
    // printf("\n\n--[868]---------------- phyTxPower              = %d ----------------\n", phyTxPower);

    // printf("\n------dr = %d, phyTxPower = %d dBm, User-Set-TxPower = %d------\n", txConfig->Datarate, phyTxPower, txConfig->TxPower);
