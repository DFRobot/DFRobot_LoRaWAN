 ...
 */
RTC_DATA_ATTR static RegionNvmDataGroup1_t* RegionNvmGroup1;
RTC_DATA_ATTR static RegionNvmDataGroup2_t* RegionNvmGroup2;
...
bool RegionUS915RxConfig( RxConfigParams_t* rxConfig, int8_t* datarate )
{
    ...
    *datarate = (uint8_t) dr;
    // printf("RegionUS915RxConfig()--------------frequency = %d   dr = %d\n", frequency, dr);
    return true;
}
...
bool RegionUS915TxConfig( TxConfigParams_t* txConfig, int8_t* txPower, TimerTime_t* txTimeOnAir )
{
    ...
    int8_t txPowerLimited = RegionCommonLimitTxPower(txConfig->TxPower, RegionNvmGroup1->Bands[RegionNvmGroup2->Channels[txConfig->Channel].Band].TxMaxPower);  // TODO 注意，此写法突破了协议栈最大21dbm的限制，可达到22dbm
    uint32_t bandwidth = RegionCommonGetBandwidth( txConfig->Datarate, BandwidthsUS915 );
    int8_t phyTxPower = 0;

    // Calculate physical TX power
    phyTxPower = RegionCommonComputeTxPower( txPowerLimited, US915_DEFAULT_MAX_ERP, txConfig->AntennaGain ) + 3/*损耗*/;

    // printf("RegionUS915TxConfig--------------frequency = %d   channel = %d   SF = %d\n", RegionNvmGroup2->Channels[txConfig->Channel].Frequency, txConfig->Channel, phyDr);
...
LoRaMacStatus_t RegionUS915NextChannel( NextChanParams_t* nextChanParams, uint8_t* channel, TimerTime_t* time, TimerTime_t* aggregatedTimeOff )
{
    ...
    // printf("RegionNvmGroup1->ChannelsMaskRemaining: ");
    // for (int i = 0; i < REGION_NVM_CHANNELS_MASK_SIZE; i++) {
    //     printf("%04X ", RegionNvmGroup1->ChannelsMaskRemaining[i]);  // 以16进制打印每个元素
    // }
    // printf("\n");
    ...
    // printf("1-RegionNvmGroup1->JoinChannelGroupsCurrentIndex = %d\n", RegionNvmGroup1->JoinChannelGroupsCurrentIndex);
    ...
    if( status == LORAMAC_STATUS_OK )
    {
        if( nextChanParams->Joined == true )
        {
            ...
            *channel = enabledChannels[randr( 0, nbEnabledChannels - 1 )];
            // printf("RegionUS915NextChannel1--------------channel = %d\n", *channel);
        }
        else
        {
            ...
            if( nextChanParams->Datarate == DR_0 )
            {
                ...
                // printf("2-RegionNvmGroup1->JoinChannelGroupsCurrentIndex = %d\n", RegionNvmGroup1->JoinChannelGroupsCurrentIndex);
            }
            ...
            else
            {
                ...
                *channel = 64 + i;
                // printf("RegionUS915NextChannel2--------------channel = %d\n", *channel);
            }
        }
