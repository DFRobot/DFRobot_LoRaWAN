...
/*
 * Number of security context entries           安全上下文条目的数量
 */
 ...
 */
RTC_DATA_ATTR static LoRaMacCryptoNvmData_t* CryptoNvm;
...
/*!
 * Verifies cmac with adding B0 block in front.     用前面添加B0块来验证CMAC
 *
 * \param[IN]  msg            - Message to compute the integrity code
 * \param[IN]  len            - Length of message
 * \param[IN]  keyID          - Key identifier
...
LoRaMacCryptoStatus_t LoRaMacCryptoInit( LoRaMacCryptoNvmData_t* nvm )
{
    ...
    CryptoNvm->LrWanVersion.Fields.Minor = 0;           // 由于是1.0.x版本，此处需置为0     mating
...
LoRaMacCryptoStatus_t LoRaMacCryptoHandleJoinAccept( JoinReqIdentifier_t joinReqType, uint8_t* joinEUI, LoRaMacMessageJoinAccept_t* macMsg )
{
...
#endif
    {
        ...
        retval = DeriveSessionKey10x( APP_S_KEY, currentJoinNonce, netID, nonce );      // 在这里计算出APPSKEY
        if( retval != LORAMAC_CRYPTO_SUCCESS )
        {
            return retval;
        }

        retval = DeriveSessionKey10x( NWK_S_ENC_KEY, currentJoinNonce, netID, nonce );  // 在这里计算出NWKSKEY(网络加密密钥)
        if( retval != LORAMAC_CRYPTO_SUCCESS )
        {
            return retval;
        }
...
LoRaMacCryptoStatus_t LoRaMacCryptoSecureMessage( uint32_t fCntUp, uint8_t txDr, uint8_t txCh, LoRaMacMessageData_t* macMsg )
{
    ...
    if( fCntUp > CryptoNvm->FCntList.FCntUp )
    {
        // printf("-----------LoRaMacCryptoSecureMessage 1  step------------\n");
        retval = PayloadEncrypt( macMsg->FRMPayload, macMsg->FRMPayloadSize, payloadDecryptionKeyID, macMsg->FHDR.DevAddr, UPLINK, fCntUp );
        if( retval != LORAMAC_CRYPTO_SUCCESS )
        {
            // printf("-----------LoRaMacCryptoSecureMessage 4  step------------\n");
            return retval;
        }
// printf("-----------LoRaMacCryptoSecureMessage 2  step------------\n");
#if( USE_LRWAN_1_1_X_CRYPTO == 1 )
...
// 解密一个消息（解密 + 完整性验证）                                     地址标识符          DEVADDR                计数标识符           下行计数                        消息
...
// printf("\n\n------LoRaMacCryptoUnsecureMessage step1------\n\n");
    if( CheckFCntDown( fCntID, fCntDown ) == false )            // FCNTUP/DOWN检查失败（新FCNT小于以前的FCNT）
    {
        return LORAMAC_CRYPTO_FAIL_FCNT_SMALLER;
    }
// printf("\n\n------LoRaMacCryptoUnsecureMessage step2------\n\n");
    LoRaMacCryptoStatus_t retval = LORAMAC_CRYPTO_ERROR;        // 先把返回值 设为未定义的错误
    KeyIdentifier_t payloadDecryptionKeyID = APP_S_KEY;         // 用APPSKEY做消息解密密钥
    KeyIdentifier_t micComputationKeyID = S_NWK_S_INT_KEY;      // 用S_NWK_S_INT_KEY来做完整性解密密钥
    KeyAddr_t* curItem;                                         // 创建一个密钥地址项

    // Parse the message                                        // 解析消息
    if( LoRaMacParserData( macMsg ) != LORAMAC_PARSER_SUCCESS ) 
    {
        return LORAMAC_CRYPTO_ERROR_PARSER;                     // 解析器报错
    }
// printf("\n\n------LoRaMacCryptoUnsecureMessage step3------\n\n");
    // Determine current security context                       确定当前安全上下文                     
    retval = GetKeyAddrItem( addrID, &curItem );
    if( retval != LORAMAC_CRYPTO_SUCCESS )
    {
        return retval;
    }
// printf("\n\n------LoRaMacCryptoUnsecureMessage step4------\n\n");
    payloadDecryptionKeyID = curItem->AppSkey;
    micComputationKeyID = curItem->NwkSkey;

    // Check if it is our address                               检查是否是我们的地址                
...
// printf("\n\n------LoRaMacCryptoUnsecureMessage step2------\n\n");
    ...
    // printf("\n\n------LoRaMacCryptoUnsecureMessage step2 isAck = %d------\n\n", isAck);
    if( CryptoNvm->LrWanVersion.Fields.Minor == 0 )
    {
        // printf("\n\n------LoRaMacCryptoUnsecureMessage step2 mod isAck to false------\n\n");
        // In legacy mode the IsAck parameter is forced to be false since the ConfFCnt field is not used.
        // 在旧模式下，由于不使用conffcnt字段，因此ISACK参数被迫为错误。
        isAck = false;
    }

    // Verify mic     完整性校验
    // printf("\n\n------LoRaMacCryptoUnsecureMessage step macMsg->MIC = 0x%8X  isACK = %d------\n\n", macMsg->MIC, isAck);
...
// printf("\n\n------LoRaMacCryptoUnsecureMessage step3------\n\n");
    // Decrypt payload      解密消息
    if( macMsg->FPort == 0 )
    {
        // Use network session encryption key       如果是端口0则用ENC密钥解密
        payloadDecryptionKeyID = NWK_S_ENC_KEY;
    }
...
// printf("\n\n------LoRaMacCryptoUnsecureMessage step7------\n\n");
#if( USE_LRWAN_1_1_X_CRYPTO == 1 )
    if( CryptoNvm->LrWanVersion.Fields.Minor == 1 )
    {
...
void LoRaMacPayloadEncrypt(const uint8_t *buffer, uint16_t size, const uint8_t *key, uint32_t address, uint8_t dir, uint32_t sequenceCounter, uint8_t *encBuffer)
{
    ...
    uint8_t aBlock[] = {0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
    uint8_t sBlock[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
