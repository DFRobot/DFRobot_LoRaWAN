...
void SX126xInit( DioIrqHandler dioIrq )
{
    ...
    // SX126xIoTcxoInit( );                 // 需要将下面代码封装到该注释函数，先注释   mating
    ...
    SX126xSetDio2AsRfSwitchCtrl(true);      // 需要将下面代码封装到该注释函数，先注释   mating
...
void SX126xInit2( DioIrqHandler dioIrq )
{
    ...
    // SX126xIoTcxoInit( );                 // 需要将下面代码封装到该注释函数，先注释   mating
    CalibrationParams_t calibParam;
    SX126xSetDio3AsTcxoCtrl2(TCXO_CTRL_3_3V, RADIO_TCXO_SETUP_TIME << 6);
    calibParam.Value = 0x7F;
    SX126xCalibrate(calibParam);
...
void SX126xSetRfFrequency( uint32_t frequency )     // mating mod
{   
    uint8_t buf[4];
