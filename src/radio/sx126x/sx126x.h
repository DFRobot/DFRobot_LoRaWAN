...
/*!
 * \brief Declares the oscillator in use while in standby mode      在待机模式下声明正在使用的振荡器
 *
 * Using the STDBY_RC standby mode allow to reduce the energy consumption   使用STDBY_RC待机模式可以降低能耗
 * STDBY_XOSC should be used for time critical applications                 STDBY_XOSC应该用于时间要求严格的应用程序
 */
typedef enum
{
    STDBY_RC                                = 0x00,
...
/*!
 * \brief Represents a calibration configuration   表示校准配置 
 */
typedef union
{
    struct
...
void SX126xInit2( DioIrqHandler dioIrq );
...
/*!
 * \brief Sets the radio in configuration mode  设置天线为配置模式
 *
...
/*!
 * \brief Sets the radio in continuous wave transmission mode   将天线设置为连续波传输模式
 */
...
/*!
 * \brief Calibrates the given radio block      校准给定的无线电块
 *
...
/*!
 * \brief Indicates if the Radio main clock is supplied from a tcxo     配置DIO3温度补偿电路供电电压和超时时间
 *
...
void SX126xSetDio3AsTcxoCtrl2( RadioTcxoCtrlVoltage_t tcxoVoltage, uint32_t timeout );

/*!
 * \brief Sets the RF frequency 设置射频频率
 *
